...
export default function SeparationResignationPage() {
    const [data, setData] = useState<any[]>([
        {
                "id": 1,
                "startDate": "N/A",
                "releaseDate": "N/A",
                "duration": "Active Employee",
                "clearance": "No Active Separation",
                "status": "Active",
    ...
    const columns: Column[] = [
        ...
        { id: 'startDate', label: 'Notice Start Date', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.startDate}</span> },
        { id: 'releaseDate', label: 'Expected Release Date', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.releaseDate}</span> },
        { id: 'duration', label: 'Notice Duration', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.duration}</span> },
        { id: 'clearance', label: 'Clearance Status', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.clearance}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Notice Start Date</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.startDate : ''} placeholder="e.g. 2026-08-01" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Expected Release Date</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.releaseDate : ''} placeholder="e.g. 2026-08-31" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Notice Duration</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.duration : ''} placeholder="e.g. 30 Days" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Clearance Status</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.clearance : ''} placeholder="e.g. Pending IT Clearance" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Separation & Resignation</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Submit resignation letter, notice period tracking, and clearance handover.</p>
                </div>
                <Button 
            ...
            <DataTable 
                data={filteredData} 
                columns={columns}
                searchPlaceholder="Search separation status..."
