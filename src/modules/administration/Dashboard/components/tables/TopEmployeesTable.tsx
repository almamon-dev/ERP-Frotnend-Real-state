...
export default function TopEmployeesTable({ onViewAll }: { onViewAll?: () => void }) {
  ...
  const handleViewAll = () => {
    if (onViewAll) {
      ...
      navigate('/admin/analytics?tab=employee');
    }
  };
    ...
    <div className="bg-white p-4 rounded-lg border border-slate-200/90 shadow-2xs flex flex-col justify-between h-full">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          className="text-[11.5px] font-semibold text-emerald-600 hover:underline bg-transparent border-none p-0 cursor-pointer"
        >
          View All
        </button>
      </div>
