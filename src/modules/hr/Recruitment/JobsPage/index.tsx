...
export default function JobOpeningsPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'name', label: 'Job Title', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.name}</span> },
        { id: 'dept', label: 'Department', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.dept}</span> },
        { id: 'vacancies', label: 'Vacancies', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.vacancies}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
        ...
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. Full Stack Developer" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.dept : ''} placeholder="e.g. Engineering" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Vacancies</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.vacancies : ''} placeholder="e.g. 3 Openings" className="h-8 text-[12.5px]" />
                </div>
            </div>
