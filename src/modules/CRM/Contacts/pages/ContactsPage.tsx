...
export default function ContactsPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'company', label: 'Company', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.company}</span> },
        { id: 'title', label: 'Job Title', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.title}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Company</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.company : ''} placeholder="e.g. Apex Holdings" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Job Title</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.title : ''} placeholder="e.g. VP of Procurement" className="h-8 text-[12.5px]" />
                </div>
            </div>
        ...
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    ...
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Individual key personnel and client stakeholders.</p>
                </div>
                <Button 
