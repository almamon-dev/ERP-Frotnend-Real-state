...
export default function LeadsPage() {
    const [data, setData] = useState<any[]>([
        ...
        {
                "id": 2,
                "name": "Global Systems Inquiry",
                "email": "info@globalsys.io",
        ...
        {
                "id": 3,
                "name": "Apex Enterprise Project",
                "email": "sales@apex.org",
    ...
    const columns: Column[] = [
        ...
        { id: 'name', label: 'Lead Name', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.name}</span> },
        { id: 'email', label: 'Email Address', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.email}</span> },
        { id: 'phone', label: 'Phone Number', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.phone}</span> },
        { id: 'source', label: 'Source', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.source}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Lead Name</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. John Doe / TechCorp" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.email : ''} placeholder="e.g. john@techcorp.com" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Phone Number</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.phone : ''} placeholder="e.g. +1 555-0192" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Source</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.source : ''} placeholder="e.g. Website / Referral" className="h-8 text-[12.5px]" />
                </div>
            </div>
        ...
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    ...
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Track, assign, and convert potential business prospects.</p>
                </div>
                <Button 
