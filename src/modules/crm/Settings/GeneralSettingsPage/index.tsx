...
export default function GeneralCRMSettingsPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'value', label: 'Config Value', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.value}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
        ...
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. Default Currency" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Config Value</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.value : ''} placeholder="e.g. USD ($)" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">General CRM Settings</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Default currency, pipeline stages, and system preferences.</p>
                </div>
                <Button 
            ...
            <Modal
                ...
                title="Create General CRM Settings"
                size="lg"
                footer={
                    <>
            ...
            <Modal
                ...
                title="Edit General CRM Settings"
                size="lg"
                footer={
                    <>
