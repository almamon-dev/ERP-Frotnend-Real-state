...
export default function OpportunitiesDealsPage() {
    const [data, setData] = useState<any[]>([
        {
                "id": 1,
                "name": "Acme SaaS License Renewal",
                "value": "$50,000",
                "stage": "Proposal Sent",
                "status": "Active",
    ...
    const columns: Column[] = [
        ...
        { id: 'name', label: 'Deal Name', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.name}</span> },
        { id: 'value', label: 'Expected Value', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.value}</span> },
        { id: 'stage', label: 'Stage', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.stage}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Deal Name</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. Cloud Migration Deal" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Expected Value</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.value : ''} placeholder="e.g. $45,000" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Stage</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.stage : ''} placeholder="e.g. Negotiation" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Opportunities & Deals</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Sales deal pipeline tracking with revenue forecasting.</p>
                </div>
                <Button 
            ...
            <Modal
                ...
                title="Create Opportunities & Deals"
                size="lg"
                footer={
                    <>
            ...
            <Modal
                ...
                title="Edit Opportunities & Deals"
                size="lg"
                footer={
                    <>
