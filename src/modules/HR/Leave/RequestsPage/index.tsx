...
export default function LeaveRequestsPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'leaveType', label: 'Leave Type', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.leaveType}</span> },
        { id: 'days', label: 'Total Days', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.days}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Leave Type</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.leaveType : ''} placeholder="e.g. Casual Leave" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Total Days</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.days : ''} placeholder="e.g. 3 Days" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Leave Requests</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Process employee leave applications and manager approvals.</p>
                </div>
                <Button 
