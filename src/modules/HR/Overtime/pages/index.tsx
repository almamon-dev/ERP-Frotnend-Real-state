...
export default function OvertimeTrackingPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'hours', label: 'Overtime Hours', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.hours}</span> },
        { id: 'approver', label: 'Approved By', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.approver}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
        ...
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. Carlos Mendez" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Overtime Hours</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.hours : ''} placeholder="e.g. 3.5 Hours" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Approved By</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.approver : ''} placeholder="e.g. Emily Watson" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Overtime Tracking</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Track extra working hours, approvals, and overtime rates.</p>
                </div>
                <Button 
            ...
            <Modal
                ...
                title="Create Overtime Tracking"
                size="lg"
                footer={
                    <>
