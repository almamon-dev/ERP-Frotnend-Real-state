...
export default function AppraisalReviewsPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'name', label: 'Cycle Title', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.name}</span> },
        { id: 'emp', label: 'Employee Name', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.emp}</span> },
        { id: 'rating', label: 'Rating Score', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.rating}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Appraisal Reviews</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">360-degree performance evaluation cycles and reviews.</p>
                </div>
                <Button 
            ...
            <Modal
                ...
                title="Create Appraisal Reviews"
                size="lg"
                footer={
                    <>
