...
export default function SalaryStructuresPage() {
    const [data, setData] = useState<any[]>([
        {
                "id": 1,
                "name": "Senior Management Pay Scale",
                "basic": "55%",
                "status": "Active",
        ...
        {
                "id": 2,
                "name": "Mid-Level Professional Scale",
                "basic": "50%",
                "status": "Active",
                "createdAt": "2026-01-01"
        }
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Grade / Scale</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. Grade A - Executive" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Basic Percentage</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.basic : ''} placeholder="e.g. 50%" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Salary Structures</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Define basic pay, HRA, allowances, and tax withholding slabs.</p>
                </div>
                <Button 
