...
export default function MonthlyPayrollProcessingPage() {
    const [data, setData] = useState<any[]>([
        {
                "id": 1,
                "period": "June 2026 Payroll",
                "count": "142 Employees",
                "amount": "$182,500",
                "status": "Disbursed",
    ...
    const columns: Column[] = [
        ...
        { id: 'period', label: 'Pay Period', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.period}</span> },
        { id: 'count', label: 'Total Employees', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.count}</span> },
        { id: 'amount', label: 'Total Outflow', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.amount}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Pay Period</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.period : ''} placeholder="e.g. July 2026" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Total Employees</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.count : ''} placeholder="e.g. 145" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Total Outflow</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.amount : ''} placeholder="e.g. $185,000" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Monthly Payroll Processing</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Process gross salaries, tax deductions, bonuses, and net pay.</p>
                </div>
                <Button 
            ...
            <Modal
                ...
                title="Create Monthly Payroll Processing"
                size="lg"
                footer={
                    <>
            ...
            <Modal
                ...
                title="Edit Monthly Payroll Processing"
                size="lg"
                footer={
                    <>
