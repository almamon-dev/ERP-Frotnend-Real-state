...
export default function ProvidentFundGratuityPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'pfNo', label: 'PF Account No', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.pfNo}</span> },
        { id: 'balance', label: 'Total Balance', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.balance}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
        ...
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                ...
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. Emily Watson" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">PF Account No</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.pfNo : ''} placeholder="e.g. PF-99482" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Total Balance</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.balance : ''} placeholder="e.g. $14,500" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Provident Fund & Gratuity</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Manage company contribution funds, gratuity balances, and health benefits.</p>
                </div>
                <Button 
            ...
            <DataTable 
                data={filteredData} 
                columns={columns}
                searchPlaceholder="Search benefit funds..."
            ...
            <Modal
                ...
                title="Create Provident Fund & Gratuity"
                size="lg"
                footer={
                    <>
            ...
            <Modal
                ...
                title="Edit Provident Fund & Gratuity"
                size="lg"
                footer={
                    <>
