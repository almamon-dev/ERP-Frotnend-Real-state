...
export default function InterviewSchedulesPage() {
    ...
    const columns: Column[] = [
        ...
        { id: 'name', label: 'Applicant Name', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.name}</span> },
        { id: 'interviewer', label: 'Interviewer', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.interviewer}</span> },
        { id: 'time', label: 'Scheduled Time', render: (item: any) => <span className="font-bold text-slate-800 text-[13px]">{item.time}</span> },
        {
            id: 'status',
            label: 'Status',
            render: (item) => (
    ...
    const FormContent = ({ isEdit = false }: { isEdit?: boolean }) => (
        <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Applicant Name</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.name : ''} placeholder="e.g. Daniel Lee" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Interviewer</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.interviewer : ''} placeholder="e.g. Robert Smith" className="h-8 text-[12.5px]" />
                </div>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2.5">
                <FormLabel className="!mb-0 sm:w-32 text-[12.5px] font-semibold text-slate-700 shrink-0">Scheduled Time</FormLabel>
                <span className="text-[12.5px] text-slate-400 hidden sm:inline">:</span>
                <div className="flex-1">
                    <Input defaultValue={isEdit ? editItem?.time : ''} placeholder="e.g. 2026-07-28 02:00 PM" className="h-8 text-[12.5px]" />
                </div>
            </div>
    ...
    return (
        <div className="p-4 md:p-6 max-w-full mx-auto bg-[#f8f9fa] min-h-screen pb-20">
            <div className="flex justify-between items-center mb-5">
                <div>
                    <h1 className="text-[20px] font-bold text-slate-900">Interview Schedules</h1>
                    <p className="text-[13px] font-medium text-[#008060] mt-0.5">Schedule technical & HR interview rounds with interviewers.</p>
                </div>
                <Button 
            ...
            <Modal
                ...
                title="Create Interview Schedules"
                size="lg"
                footer={
                    <>
            ...
            <Modal
                ...
                title="Edit Interview Schedules"
                size="lg"
                footer={
                    <>
